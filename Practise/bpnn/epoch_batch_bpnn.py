...
def create_model():
    model = Sequential() 
    model.add(Dense(64,input_dim=nf,kernel_initializer='uniform',))
    model.add(Activation('relu'))
    model.add(Dense(32,input_dim=64,kernel_initializer='uniform',))
    model.add(Activation('relu'))
    model.add(Dense(16,input_dim=32,kernel_initializer='uniform',))
    model.add(Activation('relu'))
    model.add(Dense(1,input_dim=16,kernel_initializer='uniform')) 
    rmsprop = optimizers.RMSprop(lr=0.001, rho=0.9, epsilon=1e-06)
    model.compile(loss='mse', optimizer='rmsprop') 
    return model
model = KerasRegressor(build_fn=create_model,verbose=0)
batch_size = [60,100,140,180,220]
...
"""summarize results
"""
R  = np.zeros((len(batch_size)*len(epochs),3))
nn = 0
print("Best: %f using %s" % (grid_result.best_score_, grid_result.best_params_))
for params, mean_score, scores in grid_result.grid_scores_:
    print("%f (%f) with: %r" % (scores.mean(), scores.std(), params))
    R[nn][0] = scores.mean()
    R[nn][1] = params['batch_size']
    R[nn][2] = params['nb_epoch']
    nn = nn + 1
