...
def create_model(dropout_rate=0.0, weight_constraint=0):
    model = Sequential() 
    model.add(Dense(64,input_dim=nf,kernel_initializer='uniform',W_constraint=maxnorm(weight_constraint)))
    model.add(Activation('linear'))
    model.add(Dense(32,input_dim=64,kernel_initializer='uniform',W_constraint=maxnorm(weight_constraint)))
    model.add(Activation('linear'))
    model.add(Dense(16,input_dim=32,kernel_initializer='uniform',W_constraint=maxnorm(weight_constraint)))
    model.add(Activation('linear'))
    model.add(Dropout(dropout_rate))
    model.add(Dense(1,input_dim=16,kernel_initializer='uniform',W_constraint=maxnorm(weight_constraint))) 
...
"""summarize results
"""
R  = np.zeros((len(weight_constraint)*len(dropout_rate),3))
nn = 0
print("Best: %f using %s" % (grid_result.best_score_, grid_result.best_params_))
for params, mean_score, scores in grid_result.grid_scores_:
    print("%f (%f) with: %r" % (scores.mean(), scores.std(), params))
    R[nn][0] = scores.mean()
    R[nn][1] = params['weight_constraint']
    R[nn][2] = params['dropout_rate']
    nn = nn + 1
